classdef noiseMPC < handle
	...
	methods
		% Constructor
		function obj = noiseMPC(model, x0, Tend, N, Q, R, x_ref, u_ref, preview, formulation, noise, debug)
		...
		function [x, u] = optimize(obj)
            ...
            % #TODO: CHange here --> state with noise
			...
			for k = 1:obj.Nsteps
                ...
                % #TODO: CHange here --> state estimation with kalman filter
                %        give the optimal input

				% x_last = obj.model.simulate(x_last, u_optimal, obj.Ts);

                update_last = obj.model.simulate(x_last, u_optimal, obj.Ts) + w(k,:)';
                y = obj.model.output(update_last, u_optimal) + v(k,:)';
                x_last = obj.model.EKF_step(x_last, u_optimal, y);
